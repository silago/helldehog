...
func createTileset(var data, var cell_size, collidable):
	var ts = TileSet.new()
	var size = cell_size
	for t in data:
							...
							for obj in tiles[tile]["objectgroup"]["objects"]:
								if !obj.has("polyline") and !obj.has("polygon") and !obj.has("ellipse"):
									...
									if w == h:
										...
										ts.tile_set_shape_offset(count, Vector2(w/2 + xx, h/2 + yy))
								elif obj.has("polygon") and collidable:
...
func _on_Button_pressed():
	...
	var ctileset = createTileset(tileset_data, cell_size,true)
	var utileset = createTileset(tileset_data, cell_size,false)

	if(!ctileset and !utileset):
